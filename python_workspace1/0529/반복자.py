...
1. 선형구조 - 배열과 링크드 리스트
    1-1. 배열
         연속된 메모리 공간을 필요로 한다.
         주기억장치(ram)에 연속된 공간이 없으면 할당을 못함
         100m 필요하면 100m를 줄수 있어야 한다.
         10,20,40,30 조각- 단편화
         압축을 해서 -> 100m 을 만들어서 배당(os가)
         데이터를 접근할떄 index를 사용한다.
         본래의 배열은 프로그램 시작전에 메모리크기를 정확하게 지정해야 하낟.
         그리고 중간에 바꿀 수 없다.
         a = []               a[0]= 10 print( a[1])
         a.append(1)
         a.append(2)
         현재는 배열이라고 부르는것들이 인덱스를 사용한다는 공통점 하나만 남았음
         배열을 장점은 첫시작위치를 알면 다음번 요소가 바로옆에 있다.
    ...
    1-2 링크드리스트
        ...
        목걸이공예 - 비즈를 가지고 연결시켜나가는ㄴ 개념
        데이터+주소
        (데이터ㅣ주소)->(데이터주소)->(데이터주소)
        필요한 만큼만 만들어서 사용 가능하다
...
print(next(it2))
print(next(it2))
#반복자 목적 : 사용자가 클래스 내부를 몰라도 동일한 방법으로 접근할 수있게
...
while True:
    try:
        item =next(it)#현재 가르키는 데이터 반환하고 다음요소로 이동
        print(item)
    except:
        ("이터레이터종료")
        break
