import time
# @staticmethod처럼 @시작한다.
#함수의 전후동작을 감싸서 함수 가로채기를 해서 뭔가 작업을
#처리하고 원래 함수를 호출한다.

#데코레이터는 함수안에 또 함수를 만든다. (중첩함수)
#매개변수로 함수를 받아가서 중첩함수 안에서 받아간 함수를 호출한다.
#파이썬에서는 주로 웹에서 많이 사용한다.
#반드시 로그온을 해야 접근이 가능한 함수를 만들고 싶다.
#
# def decorater1(func):#매개변수는 중첩함수안에서 호출될 함수
#     def wrapper():
#         print("함수호출전")
#         func()
#         print("함수호출후")
...
# hello() #decorator1에게 납치당함 -> wrapper ->func() 를 통해함수호출
...
def mydecorator(callback):
    #호출될 함수가 어떤 매개변수를 갖고 있는지 알수 없을경우에
    #tuple타입, dict 타입
